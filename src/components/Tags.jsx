...
function Tags ({ setCurrentPage }) {
  ...
  return (
    <div className='mt-4 flex'>
      <ul className='text-md flex overflow-x-auto p-1 pl-[2px] text-center font-medium text-gray-100 mx-auto snap-x'>
        {tags.map((tag) => (
          <li
            key={tag}
            onClick={(e) => handleTags(e, tag)}
            className={`tag mr-2 inline-block rounded-3xl bg-gray-800 py-2 px-3 text-white snap-center ${
              active === tag
